        ...
        var listTemp = function(index, d){
            /*{
                ...
                product: {
                    ...
                    info: '颜色：黄白  尺寸：小号',
                    url: '/detail.html',
                    image: '/www/images/detail/2s.jpg'
                },
                price: '128.00',
                num: '1',
                total: '128.00',
            ...
            var outer = (num === 0 || maxNum === 0) ? '<div class="js-stock-tips c-999">'+ d.error_msg +'</div>' : '';
            var html = '<tr data-id="'+ d.id +'" data-index="'+ index +'">'+
                '    <td class="t-c"><label><input type="checkbox" class="js-rewrite js-item-checked"'+ disabled + checked +'></label></td>'+
                '    <td>'+
                '        <div class="product-info">'+
                '            <a href="'+ d.product.url +'">'+
                '                <div class="image"><img src="'+ d.product.image +'" width="98" height="98"></div>'+
                '                <div class="info">'+
                '                    <h3 class="f-14">'+ d.product.name +'</h3>'+
                '                    <p class="c-666">'+ d.product.info +'</p>'+
                '                </div>'+
                '            </a>'+
                '        </div>'+
                '    </td>'+
                '    <td valign="top" class="c-e73d4d">'+ d.price +'</td>'+
                '    <td valign="top">'+
                '        <div class="moyu-input-number '+ disabled +'" data-min="'+ d.min_num +'" data-max="'+ d.max_num +'">'+
                '            <a href="javascript:;" class="cut">-</a>'+
                '            <input type="text" value="'+ d.num +'" disabled data-mgs="'+ d.error_msg +'">'+
                '            <a href="javascript:;" class="add">+</a>'+
                '        </div>'+ outer +
                '    </td>'+
                '    <td valign="top" class="c-e73d4d js-item-total">'+ d.total +'</td>'+
                '    <td valign="top"><a href="javascript:;" class="js-del">删除</a></td>'+
                '</tr>';
            return html;
        };
        var html = '';
        ...
        $.subscribe('change.cart', function(){
            ...
            if(d.type === 'num'){
                if(typeof(index) !== 'undefined'){
                    ...
                    box.find('tr[data-id="'+ d.id +'"]').find('.js-item-total').text(data[index].total);
                    countPrice();
                }
                ...
                data.splice(index, 1);
                box.find('tr[data-id="'+ d.id +'"]').remove();
                countChecked();
                countPrice();
                if(data.length === 0){
