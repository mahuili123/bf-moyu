;(function($, window, document, undefined){
    ...
    //相册切换
    var albumAct = function(){
        var album =  $('#js-album');
        var bigImg = album.find('.big-img img');
        var small = album.find('.imgs-silder ul');
        var len = small.find('li').length;
        var pager = [];
        var pre = 5;
        var width = 105;
        var pLen = Math.ceil(len/pre);
        var prev = album.find('.prev');
        var next = album.find('.next');
        var current = 1;
        var changePosition = function(){
            var index = pager[current-1][0] - 1;

            //var me = small.find('li').eq(index);
            small.animate({
                left: width * (-index)
        ...
        //点击小图切换大图
        small.find('a').on('click', function(){
            var me = $(this);
            var src = me.attr('href');
            bigImg.attr('src', src);
            me.closest('li').addClass('current').siblings('li').removeClass('current');
            return false;
        });
    };
    ...
    var MyProduct = function(){
        ...
        this.item = function(d){
            ...
            var html = '<li class="js-dim-item '+ checked +' '+ disabled +'" data-id="'+ d.id +'" data-pid="'+ d.pid +'" data-min="'+ d.minPrice +'" data-max="'+ d.maxPrice +'" data-pindex="'+ d.pindex +'" data-index="'+ d.index +'"><a href="javascript:;">'+ d.name +'<i></i></a></li>';
            return html;
        };
        this.temp = function(){
            var html = '';
